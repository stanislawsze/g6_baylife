<div>
    <div class="bg-gray-800 rounded-3xl p-5 grid grid-cols-4 gap-4">
        <div class="text-sm text-gray-200 p-1">
            Mission en cours : <b>{{$mission ?? 'Non démarré'}}</b>
        </div>
        <div class="text-sm text-gray-200 p-1">
            Début de service : <b>{{ $startTime ?? 'Non démarré' }}</b>
        </div>
        <div class="text-sm text-gray-200 p-1">
            Fin de service : <b> @if($startTime) {{ $endTime ?? 'En cours' }} @else Non démarré @endif </b>
        </div>
        <div class="text-sm text-gray-200 p-1">
            Durée : <b><span id="timer">{{$duration ?? '00:00:00'}}</span></b>
        </div>
    </div>
    <div class="bg-gray-800 rounded-3xl p-5 my-4 text-gray-200">
        <div class="grid grid-cols-3 gap-4 my-4">
            <div>
                Type de mission
                <label for="Toggle4" class="rounded-3xl inline-flex items-center p-1 cursor-pointer bg-g6-3 text-gray-200">
            ...
            <div>
                ...
                <select id="vehicles" wire:model.change="vehicleTaken" class="block w-full rounded-md border-0 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:max-w-xs sm:text-sm sm:leading-6">
        ...
        <div class="w-full flex justify-center">
            <button wire:click="startStopTimer" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-200 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                {{ $isRunning ? 'Arrêter' : 'Démarrer' }}
            </button>
        </div>
    </div>
    <div class="my-5 bg-gray-800 text-gray-200 rounded-3xl p-5" wire:poll.visible>
    ...
    <div class="my-5 bg-gray-800 dark:text-gray-200 rounded-3xl p-5">
