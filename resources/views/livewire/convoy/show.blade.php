<div >
    <div class="rounded-3xl p-4 bg-white my-2">
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                <button {{auth()->user()->hasEntryInConvoyTable($convoy->id) ? 'disabled' : ''}} wire:click="joinConvoy({{$convoy->id}})"
                        class="rounded border p-1 hover:bg-green-500">Rejoindre le convoi</button>
            </div>
        </div>
    </div>
    <div class="flex gap-4">
        <div class="w-2/3">
            ...
            <div class="my-2 rounded-3xl p-4 bg-white">
                        ...
                        <tr class="bg-white border-b transition duration-300 ease-in-out">
                            ...
                            <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                                @if($convoy->userVehicle($user->id))
                                    <select disabled id="vehicles" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                ...
                                @else
                                <select wire:change="updateUserVehicle('{{$user->id}}')" wire:model.change="vehicleId" id="vehicles" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                    @foreach($vehicles as $v)
                            ...
                            <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                                <input type="number" wire:model.blur="userSalary['{{$user->id}}']" class="w-full rounded-md focus:ring focus:ri focus:ri">
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
        ...
        <div class="w-1/3">
            <div class="rounded-3xl p-4 bg-white my-2 fixed">
                <div class="col-span-full sm:col-span-3">
                    <label for="nightNumber" class="text-sm">Nombre de Nightshark</label>
                    <input id="nightNumber" wire:model.blur="nbreNightshark" type="number" placeholder="2" class="w-full rounded-md focus:ring focus:ri focus:ri">
                </div>
                <div class="col-span-full sm:col-span-3">
                    <label for="stockNumber" class="text-sm">Nombre de Stockade</label>
                    <input id="stockNumber" wire:model.blur="nbreStockade" type="number" placeholder="2" class="w-full rounded-md focus:ring focus:ri focus:ri">
                </div>
                <div class="col-span-full sm:col-span-3">
                    <label for="u2rNumber" class="text-sm">Nombre d'U.2.R</label>
                    <input id="u2rNumber" wire:model.blur="nbreU2R" type="number" placeholder="2" class="w-full rounded-md focus:ring focus:ri focus:ri">
                </div>
                <div class="col-span-full sm:col-span-3">
                    <label for="name" class="text-sm">Nom du convoi</label>
                    <input id="name" wire:model.blur="convoyName" type="text" placeholder="ALTA DE TES MORTS" class="w-full rounded-md focus:ring focus:ri focus:ri">
                </div>
                <div class="col-span-full sm:col-span-3">
                    <label for="date" class="text-sm">Date du convoi</label>
                    <input id="date" wire:model.blur="startAt" type="datetime-local" placeholder="{{now()}}" class="w-full rounded-md focus:ring focus:ri focus:ri ">
                </div>
                <div class="col-span-full sm:col-span-3">
                    <label for="amount" class="text-sm">Montant du convoi</label>
                    <input id="amount" wire:model.blur="convoyAmount" type="number" placeholder="2" class="w-full rounded-md focus:ring focus:ri focus:ri">
                </div>
